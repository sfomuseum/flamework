<?php
	...
	if (post_isset('delete') && crumb_check($crumb_key) && (! $key_row['disabled'])){
		...
		if ($conf){
			...
			$GLOBALS['smarty']->assign("delete_rsp", $rsp);
		}		

		$GLOBALS['smarty']->assign("key", $key_row);

		$GLOBALS['smarty']->display("page_api_oauth2_application_delete.txt");
		exit();
	}
	...
	if ($token_row = api_oauth2_access_tokens_get_for_user_and_key($GLOBALS['cfg']['user'], $key_row)){
		$GLOBALS['smarty']->assign("self_token", $token_row);
		$GLOBALS['smarty']->assign("has_self_token", 1);
	}

	# TO DO: get API keys stats, etc.

	$GLOBALS['smarty']->display("page_api_oauth2_application.txt");
	exit();
